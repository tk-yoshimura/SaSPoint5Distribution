...
namespace AlphaPoint5Distribution {
    internal class PadeApproximation {
        static void Main() {
            ...
            for (double xmin = 1d / 4096; Math.ILogB(xmin) < 128; xmin *= 2) {
                ranges.Add((xmin, xmin * 2));
            }

            using (StreamWriter sw = new("../../../../results_disused/pade_pdf_precision120_3.csv")) {
                foreach ((double xmin, double xmax) in ranges) {
                    ...
                    for (int m = 16; m <= 64; m++) {
                        ...
                        if (max_rateerr < 1e-120 && param.All(item => item.val.Sign == Sign.Plus)) {
